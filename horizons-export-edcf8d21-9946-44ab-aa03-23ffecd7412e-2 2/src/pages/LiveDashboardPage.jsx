...
const LiveDashboardPage = () => {
  ...
  const [undoIntervalId, setUndoIntervalId] = useState(null);
  ...
  const goOffline = useCallback(() => {
    ...
    toast({ title: "You are now Offline!", description: "Your session has ended." });
  }, [requestCap, allSongTags, toast]);
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="grid lg:grid-cols-3 gap-8">
                    ...
                    <AnimatePresence>
                      {undoVisible && (
                        <motion.div initial={{opacity: 0, height: 0}} animate={{opacity: 1, height: 'auto'}} exit={{opacity: 0, height: 0}} className="bg-yellow-900/50 border border-yellow-700 rounded-lg p-4 flex items-center justify-between mb-4">
                          <p className="text-yellow-300">Going offline in {undoTimer}s...</p>
              ...
              <div className="space-y-8">
                <Card className="glass-effect border-gray-800"><CardHeader><CardTitle>Controls</CardTitle></CardHeader><CardContent className="space-y-4"><div><div className="flex items-center justify-between"><Label htmlFor="request-cap">Request Cap</Label><Tooltip><TooltipTrigger asChild><button><Info className="w-4 h-4 text-gray-500"/></button></TooltipTrigger><TooltipContent><p className="max-w-xs">Limit how many active requests can be in your Live Request Queue. Be mindful of how long you have in your set, and ensure all requests will be fulfilled.</p></TooltipContent></Tooltip></div><Input id="request-cap" type="number" value={requestCap} onChange={handleCapChange} className="bg-gray-900 border-gray-700" /></div><p className="text-sm text-gray-400">Requests in Queue: <span className="font-bold text-white">{groupedPendingRequests.length} / {requestCap > 0 ? requestCap : '∞'}</span></p></CardContent></Card>
                <Card className="glass-effect border-gray-800"><CardHeader><div className="flex items-center justify-between"><CardTitle className="flex items-center"><Tag className="mr-3 text-blue-400"/>Style/Genres</CardTitle><Tooltip><TooltipTrigger asChild><button><Info className="w-4 h-4 text-gray-500"/></button></TooltipTrigger><TooltipContent><p className="max-w-xs">As your Set progresses, you can make only a subset of songs appear for your audience on your Live Page. Allowing you to change the pacing/style of your set, whilst still allowing songs to be requested and fulfilled.</p></TooltipContent></Tooltip></div><CardDescription>Filter songs shown on your public page.</CardDescription></CardHeader><CardContent><div className="flex flex-wrap gap-2">{allSongTags.length > 0 ? allSongTags.map(tag => (<Button key={tag} variant={activeSongTags.includes(tag) ? 'default' : 'outline'} onClick={() => handleTagToggle(tag)} className="h-8">{tag}</Button>)) : <p className="text-sm text-gray-500">Add tags to songs in your setlist to enable filtering.</p>}</div>{allSongTags.length > 0 && <div className="text-right mt-4"><Button variant="link" size="sm" onClick={resetActiveTags} className="text-blue-400 hover:text-blue-300 p-0 h-auto">Reset/Deselect All</Button></div>}</CardContent></Card>
